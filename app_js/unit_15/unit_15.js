...
const f7 = () => {
  ...
  let s7 = new Set(inp);
  if (s7.size == inp.length && inp.length > 6) {
    out.textContent = 1;
  } else {
    out.textContent = 0;
  }
...
const f9 = (our_set) => {
  let res = [...our_set].join(" "); // нашел ещё пару способов, особенно, хотел этот попробовать
  // let res = Array.from(our_set).join(" "); /// способ 2
  // for (let item of our_set) {  // это тот, что был на поверхности
  //   res += item + " ";
  // }
  return res;
};
