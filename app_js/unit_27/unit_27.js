...
function resolveF1(data) {
  ...
  let number = data["random-number"] > 50 ? true : false;
...
function f6() {
  ...
  Promise.all([promise_1, promise_2]).then((data) => {
    console.log(data);
    let sum = 0;
    sum = data[0]["random-number"] + data[1]["random-number"];
...
function f8() {
  ...
  let race1 = document.querySelector(".s-81").value;
  let race2 = document.querySelector(".s-82").value;
  ...
  const promise_2 = new Promise((resolve) => {
    fetch(URL + "/api/27/sr/read/" + race2, {
      headers: requestHeaders,
    }).then((data) => resolve(data.json()));
  });
